...
class ScoreNet(torch.nn.Module):
    def __init__(self,config, marginal_prob_std, hidden_dim=256,device='cuda'):
        ...
        self.input_mlp = layers.MultiLayerPerceptron(1, [self.hidden_dim, self.hidden_dim], activation=self.config.mlp_act)
        self.output_mlp = layers.MultiLayerPerceptron(2*self.hidden_dim, [hidden_dim, hidden_dim//2,1], activation=config.gnn_act)
